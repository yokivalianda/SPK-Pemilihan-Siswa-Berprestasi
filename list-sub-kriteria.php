...
if(isset($_POST['tambah'])):	
	...
	$nilai = $_POST['nilai_sub_kriteria'];

	if(!$id_kriteria) {
		$errors[] = 'ID kriteria tidak boleh kosong';
	}
	// Validasi nama_sub_kriteria Kriteria
	if(!$nama_sub_kriteria) {
	...
	if(empty($errors)):
		$simpan = mysqli_query($koneksi,"INSERT INTO sub_kriteria (id_sub_kriteria, id_kriteria, nama_sub_kriteria, nilai_sub_kriteria) VALUES ('', '$id_kriteria', '$nama_sub_kriteria', '$nilai')");
...
if(isset($_POST['edit'])):	
	...
	// Validasi nama_sub_kriteria Kriteria
	if(!$nama_sub_kriteria) {
		$errors[] = 'Nama sub kriteria kriteria tidak boleh kosong';
	}		
	// Validasi Tipe
	if(!$nilai) {
		$errors[] = 'Nilai kriteria tidak boleh kosong';
	...
	if(empty($errors)):
		$update = mysqli_query($koneksi,"UPDATE sub_kriteria SET nama_sub_kriteria = '$nama_sub_kriteria', nilai_sub_kriteria = '$nilai' WHERE id_kriteria = '$id_kriteria' AND id_sub_kriteria = '$id_sub_kriteria'");
...
<div class="card shadow mb-4">
    ...
    <div class="card-header py-3">
        <div class="d-sm-flex align-items-center justify-content-between">
			<h6 class="m-0 font-weight-bold text-success"><i class="fa fa-table"></i> <?= $data['nama_kriteria']." (".$data['kode_kriteria'].")" ?></h6>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="myModalLabel"><i class="fa fa-plus"></i> Tambah <?= $data['nama_kriteria'] ?></h5>
				...
				<form action="" method="post">
					<div class="modal-body">
						...
						<div class="form-group">
							...
							<input autocomplete="off" type="text"class="form-control" name="nama_sub_kriteria" required>
						</div>
						<div class="form-group">
							<label class="font-weight-bold">Nilai</label>
							<input autocomplete="off" step="1" type="number" name="nilai_sub_kriteria" class="form-control" required>
						</div>
					</div>
			...
			<table class="table table-bordered" width="100%" cellspacing="0">
				...
				<tbody>
					<?php 
						...
						$q = mysqli_query($koneksi,"SELECT * FROM sub_kriteria WHERE id_kriteria = '$id_kriteria' ORDER BY nilai_sub_kriteria DESC");			
						while($d = mysqli_fetch_array($q)){
					?>
					<tr align="center">
						<td><?=$no ?></td>
						<td align="left"><?= $d['nama_sub_kriteria'] ?></td>
						<td><?= $d['nilai_sub_kriteria'] ?></td>
						<td>
					...
					<div class="modal fade" id="editsk<?= $d['id_sub_kriteria'] ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
						<div class="modal-dialog">
							<div class="modal-content">
								<div class="modal-header">
									<h5 class="modal-title" id="myModalLabel"><i class="fa fa-edit"></i> Edit <?= $d['nama_sub_kriteria'] ?></h5>
									<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
								</div>
								<form action="list-sub-kriteria.php?id=<?php echo $d['id_sub_kriteria']; ?>" method="post">
									...
									<div class="modal-body">
										...
										<div class="form-group">
											<label class="font-weight-bold">Nama Sub Kriteria</label>
											<input type="text" autocomplete="off" class="form-control" value="<?= $d['nama_sub_kriteria'] ?>" name="nama_sub_kriteria" required>
										</div>
										<div class="form-group">
											<label class="font-weight-bold">Nilai</label>
											<input type="number" step="0.001" autocomplete="off" name="nilai_sub_kriteria" class="form-control" value="<?= $d['nilai_sub_kriteria'] ?>" required>
										</div>
									</div>
									<div class="modal-footer">
										<button type="button" class="btn btn-warning" data-dismiss="modal"><i class="fa fa-times"></i> Batal</button>
